...
import { buildPortfolioContextPrompt, buildChatSystemPrompt, buildSentimentPrompt, buildDailySummaryPrompt } from './ai.prompts.js';
...
const AIChatHistory = mongoose.models.AIChatHistory || mongoose.model('AIChatHistory', aiChatHistorySchema);
...
export const getChatResponse = async (userId, userMessage) => {
  ...
  const recentHistory = await AIChatHistory.find({ userId }).sort({ createdAt: -1 }).limit(6).lean();
  ...
  const assistantReply = completion.choices[0]?.message?.content || 'I could not generate a response.';

  await AIChatHistory.create({ userId, role: 'user', content: userMessage });
  await AIChatHistory.create({ userId, role: 'assistant', content: assistantReply });

  return assistantReply;
};

export const getChatHistory = async (userId, limit = 20) => {
  const history = await AIChatHistory.find({ userId }).sort({ createdAt: -1 }).limit(limit).lean();
...
export const getPortfolioInsights = async (userId) => {
  ...
  const prompt = `${contextPrompt}\n\nBased on this portfolio, give 2-3 short, neutral observations about diversification or concentration risk. Do not recommend specific buy/sell actions. Keep it under 80 words.`;
